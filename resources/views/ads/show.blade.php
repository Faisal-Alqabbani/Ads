...
<div class="row ">
    <div class="col-lg-11 col-md-6 col-xs-11 m-2">
        @if(Auth::user())
            <button id="fav" data-id="{{$ad->id}}" class="{{$fav ? 'unfav':'fav'}}btn-sm btn-outline-primary waves-effect">{{$fav ? 'ازالة من المفضلة': 'اضافة الى المفضلة'}}</button>
        @endif
    ...
    <div class="col-lg-7 col-md-6 col-xs-11">
        <div class="card ">
            <div class="card-body">
                ...
                <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#myModal">تواصل مع المعلن</button>
            </div>
        </div>
    </div>
</div>
    ...
    $(document).ready(function(){
       ...
       $('#fav').on('click', function(){
            ...
            if($(this).hasClass('fav')){
                var status = 'unfav';
                var text = "ازالة من المفضلة"
            }else{
                var status = 'fav';
                var text = "اضافة الى المفضلة"
            }
